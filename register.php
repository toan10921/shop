...
// if have session customer, redirect to index.php
require_once 'ultils.php';
require_once './config.php';
require_once './classes/tbl_customer.php';
...
$connect = new Connect();
$tbl_customer = new Customer($connect);
...
if(!empty($_POST['submit'])){
    ...
    $customer_inserted = $tbl_customer->insert_customer($fullname,$email,$password,'','');
    if(is_array($customer_inserted) && count($customer_inserted) > 0){
       $_SESSION['customer'] = $customer_inserted;
       header('Location: '.Ultils::home_url('index.php'));
    }
}
